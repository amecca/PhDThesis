...
Table~\ref{tab:charges} shows the weak isospin, hypercharge and electric charge for the fundamental fermions.
...
\begin{table}[tbh]
	...
	\begin{tabular}{ c c c c }
		...
		\midrule
		\multirow{2}{*}{Quarks}  & \PQu  & \PQc   & \PQt   \\%& +$\dfrac{2}{3}$ \\
		                         & \PQd  & \PQs   & \PQb   \\%& -$\dfrac{1}{3}$ \\
		\hline
		\multirow{2}{*}{Leptons} & \PGne & \PGnGm & \PGnGt \\%& 0              \\
		                         & \Pem  & \PGmm  & \PGtm  \\%& -1             \\
		\bottomrule
	\end{tabular}
\end{table}
...
In 1964 Brout and Englert~\cite{PhysRevLett.13.321} and Higgs~\cite{PhysRevLett.13.508, HIGGS1964132} independently proposed
...
The only field that remains massless is the photon, while the other three become massive by interacting with three of the four components of the Higgs field $\Phi$ through the Brout-Englert-Higgs Mechanism.
...
\begin{table}[ht]
  ...
  \begin{tabular}{l l r l}
    ...
    CMS   \cite{CMS-SMP-15-008}&$\PW\PGg\PGg$, $\PZ\PGg\PGg$  &  8 TeV & \renewcommand{\arraystretch}{1.}\makecell[l]{
      $W\PGg\PGg$: 2.6 $\sigma$\\ $\PZ\PGg\PGg$: 5.9 $\sigma$
    } \\ \hline
    ...
    \noalign{\vspace{.3ex}}
    CMS   \cite{CMS-SMP-22-006}    & $\PW\PW\PGg$             & 13 TeV & $5.6\usep\sigma$                          \\ \hline
...
%
% SMP-22-006 & WWG, HG            & 13 TeV & 5.6 sigma                                           & https://doi.org/10.1103/PhysRevLett.132.121901
%
%
% ATLAS
...
All of these results are summarized in Table~\ref{tab:summary_triboson_papers}.
...
Hadronic branching ratios are much greater than the leptonic ones: the hadronic BR for the \PW boson is three times the one containing at least an electron or a muon, while for \PZ it is 10 times larger.
...
However, the small BR (V $\to$ leptons) translates into very low production and decay cross section of $\PZ\PZ\PGg$ and $\PW\PZ\PGg$ into leptons (Table~\ref{tab:xsection-exclusive}).
In addition the leptonic decay of a \PW boson produces a charged lepton and a neutrino, but the latter cannot be detected directly neither in CMS nor in ATLAS.
...
$\Pp\Pp \to \PZ\PZ\PGg    \to 4\Pl \PGg$,
$\Pp\Pp \to \PW\PZ\PGg    \to 3\Pl \PGn \PGg$ and
$\Pp\Pp \to \rm{V}\PZ\PGg \to 2\Pl \PGg\ 2j$ (where $\rm{V}$ is either \PZ or \PW)
respectively.

\begin{table}
  \centering
  \caption{
    Cross sections of triboson production and prediction of the number of events in a scenario where the integrated luminosity is 137 fb$^{-1}$.
    These values are derived using \MGvATNLO~\cite{MGatNLO, Frederix_2018} at next-to-leading order.
    The default kinematic selections are applied during the generation.
  }
  ...
  \begin{tabular}{ l c c }
    ...
    \midrule
    $\Pp\Pp \to \PZ\PZ\PGg$ & $ 47.4$\usep fb & $ 6\,500$ \\
    $\Pp\Pp \to \PW\PZ\PGg$ & $171.5$\usep fb & $23\,500$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}
  ...
  \begin{tabular}{ >{$}l<{$}@{}>{$}l<{$} >{$}c<{$} >{$}c<{$} } % l -> divide intermediate and final state
    \toprule
    \multicolumn{2}{c}{\text{Process}} & \text{Branching ratio}  & \text{Cross section} \\ %& Events
    \midrule
    \Pp\Pp \to \PZ\PZ\PGg & \to 4\Pl \PGg      & 0.453\unit{\%} & 0.215\unit{fb} \\ %& 29.43
    \Pp\Pp \to \PW\PZ\PGg & \to 3\Pl \PGn \PGg & 1.44 \unit{\%} & 2.46 \unit{fb} \\ %& 337.4
    \Pp\Pp \to \PZ\PZ\PGg & \to 2\Pl \PGg\ 2j  & 9.41 \unit{\%} & 4.46 \unit{fb} \\ %& 611.5
    \Pp\Pp \to \PW\PZ\PGg & \to 2\Pl \PGg\ 2j  & 4.54 \unit{\%} & 7.78 \unit{fb} \\ %& 1066
    \bottomrule
  \end{tabular}
\end{table}
