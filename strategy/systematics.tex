...
around a central value $\mu_0$ in the range $0.5\mu_0 \leq \mu_{R,F} \leq 2\mu_0$
with the constraint that $0.5 \leq \mu_R/\mu_F \leq 2$.
...
For the photons, the ratio of events of the $\PQq\PAQq\to\PZ\PZ$ background sample that pass the tight photon ID to those that pass the very loose ID
was compared to the fake rate estimated from data with the method described in Section~\ref{sec:fake_photons_background}.
To avoid introducing additional model dependencies, this uncertainty is modelled as a flat normalization factor on the \nonprompt photon contribution in the signal region,
with a magnitude of 50\unit{\%}. %, corresponding to a log-normal (see Equation~\ref{eq:lnNdef}) width of 1.4.
A similar reasoning was applied to the \nonprompt lepton contribution, resulting in a normalization uncertainty of 40\unit{\%}. % or log-normal width of 1.34
  ...
  \begin{tabular}{l >{$}c<{$} >{$}c<{$} >{$}c<{$} >{$}c<{$}}
    ...
    \multicolumn{5}{l}{
      ...
      \parbox{.95\textwidth}{
        ...
        simulation is evaluated by comparing the fake rate measured in data and the one calculated in simulation.
      }
    } \\
  \end{tabular}
\end{table}
